...
from tools.warmup_poly_lr_scheduler import WarmupPolyLR
import opts
...
def main(args):
    ...
    if args.only_cycle:
        for n, p in model_without_ddp.named_parameters():
            if not (match_name_keywords(n, 'text_query') or match_name_keywords(n, 'text_decoder')):
    ...
    param_dicts = [
        {
            "params":
                [p for n, p in model_without_ddp.named_parameters()
                 ...
                 and not match_name_keywords(n, args.lr_linear_proj_names) and not match_name_keywords(n, args.lr_anchor_names) and p.requires_grad],
            "lr": args.lr * args.lr_multi,
        },
        {
            "params": [p for n, p in model_without_ddp.named_parameters() if match_name_keywords(n, args.lr_backbone_names) and p.requires_grad],
            "lr": args.lr_backbone * args.lr_multi,
        },
        {
            ...
            "lr": args.lr_text_encoder * args.lr_multi,
        },
        {
            "params": [p for n, p in model_without_ddp.named_parameters() if match_name_keywords(n, args.lr_linear_proj_names) and p.requires_grad],
            "lr": args.lr * args.lr_linear_proj_mult * args.lr_multi,
        },
        {
            "params": [p for n, p in model_without_ddp.named_parameters() if
                       match_name_keywords(n, args.lr_anchor_names) and p.requires_grad],
            "lr": args.lr * args.lr_anchor_mult * args.lr_multi,
        }
    ]
    ...
    # max_iter = len(dataset_train) / dist.get_world_size() / args.batch_size * args.epochs
    # lr_scheduler = WarmupPolyLR(optimizer, max_iter, warmup_iters=100, warmup_factor=0.01)
    # print(f"dataset size: {len(dataset_train)}, max_iter: {max_iter}")
    ...
    # A2D-Sentences
    if args.dataset_file == 'a2d' or args.dataset_file == 'jhmdb' or args.dataset_file == 'refcocoa2d' or args.dataset_file=='refcocoVideo':
    ...
    if args.dataset_file != "davis" and args.dataset_file != "jhmdb" and args.pretrained_weights is not None:
        ...
        checkpoint = torch.load(args.pretrained_weights, map_location="cpu")
        checkpoint_dict = pre_trained_model_to_finetune(checkpoint, args)
        missing_keys, unexpected_keys = model_without_ddp.load_state_dict(checkpoint_dict, strict=False)
    ...
    if args.eval:
        assert args.dataset_file == 'a2d' or args.dataset_file == 'jhmdb' or  args.dataset_file == 'refcocoVideo', \
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, criterion, data_loader_train, optimizer, device, lr_scheduler, epoch,
            args.clip_max_norm)
        lr_scheduler.step()
        if args.output_dir:
        ...
        if args.dataset_file == 'a2d' or args.dataset_file == 'refcocoa2d':
            test_stats = evaluate_a2d(model, data_loader_val, postprocessor, device, args)
