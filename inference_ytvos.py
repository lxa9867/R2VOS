...
def sub_processor(lock, pid, args, data, save_path_prefix, save_visualize_path_prefix, img_folder, video_list):
    ...
    for vid, video in enumerate(video_list):
        ...
        for i in range(num_expressions):
            ...
            # print(torch.nn.functional.mse_loss(text_sentence_features, pseudo_text_sentence_features))
            # if vid == 10:
            #     bert_embed = torch.cat(sentence_features, dim=0)
            #     np.save(f'output/bert_embed{args.val_type}.npy', bert_embed.cpu().numpy())
            #     pseudo_bert_embed = torch.cat(pseudo_sentence_features, dim=0)
            #     np.save(f'output/pseudo_bert_embed{args.val_type}.npy', pseudo_bert_embed.cpu().numpy())
