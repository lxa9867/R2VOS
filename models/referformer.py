...
from .segmentation import CrossModalFPNDecoder, VisionLanguageFusionModule,\
    DualCrossModalFPNDecoder, RecurrentDualCrossModalFPNDecoder, DualMultiScaleCrossModalFPNDecoder
...
class ReferFormer(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
                    ...
                    freeze_text_encoder=False, rel_coord=True, fpn_type='dual', as_vos=False, query_feat_dim=2048,
                    text_enc_type='distilroberta-base', use_cycle=False, add_negative=False, is_eval=False,
                    bert_cycle=False, neg_cls=False):
        ...
        # if (self.add_negative and self.use_cycle) or self.is_eval:
        #     self.negative_anchor = nn.Embedding(1, hidden_dim)
        #     if self.bert_cycle:
        #         self.negative_anchor = nn.Embedding(1, 768)
        ...
        if not self.as_vos:
            ...
            self.tokenizer = RobertaTokenizerFast.from_pretrained(text_enc_type)
            self.text_encoder = RobertaModel.from_pretrained(text_enc_type)  # distilroberta-base 'roberta-base'
            ...
            self.query_resizer = FeatureResizer(
                input_feat_size=query_feat_dim,
                output_feat_size=hidden_dim,
                dropout=0.1,
            )

        if self.bert_cycle:
            self.cycle_projector = FeatureProjector(
        ...
        if self.fpn_type == 'dyn':
            self.pixel_decoder = DynCrossModalFPNDecoder(feature_channels=feature_channels, conv_dim=hidden_dim,
                                                      mask_dim=mask_dim, dim_feedforward=dim_feedforward, norm="GN")
        elif self.fpn_type == 'dual' or self.fpn_type == 'dual+memory_fpn':
            self.pixel_decoder = DualCrossModalFPNDecoder(feature_channels=feature_channels, conv_dim=hidden_dim,
                                                         mask_dim=mask_dim, dim_feedforward=dim_feedforward, norm="GN",
                                                          use_cycle=use_cycle, add_negative=add_negative)
        elif self.fpn_type == 'rec_dual':
            self.pixel_decoder = RecurrentDualCrossModalFPNDecoder(feature_channels=feature_channels, conv_dim=hidden_dim,
                                                         mask_dim=mask_dim, dim_feedforward=dim_feedforward, norm="GN", stage_num=3)
        elif self.fpn_type == 'ms_dual':
            self.pixel_decoder = DualMultiScaleCrossModalFPNDecoder(feature_channels=feature_channels, conv_dim=hidden_dim,
                                                          mask_dim=mask_dim, dim_feedforward=dim_feedforward, norm="GN")
        else:
        ...
        if self.neg_cls:
            self.neg_classifier = MLP(2*hidden_dim, hidden_dim, 1, 3)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        masks = []
        poses = []
        text_features, text_sentence_features, ori_text_sentence_features = self.forward_text(captions, device=pos[0].device)
        text_pos = self.text_pos(text_features).permute(2, 0, 1)  # [length, batch_size, c]
        # text_word_masks [batch_size, length]
        ...
        if self.add_negative:
            # construct negative samples for contrastive learning
            caption_list = [captions for _ in range(dist.get_world_size())]
            dist.all_gather_object(caption_list, captions)
            neg_captions = caption_list[(dist.get_rank() + 1) % dist.get_world_size()]
            neg_text_features, neg_text_sentence_features, ori_neg_text_sentence_features = self.forward_text(neg_captions, device=pos[0].device)
            neg_text_pos = self.text_pos(neg_text_features).permute(2, 0, 1)  # [length, batch_size, c]
            # text_word_masks [batch_size, length]
            neg_text_word_features, neg_text_word_masks = neg_text_features.decompose()
            neg_text_word_features = neg_text_word_features.permute(1, 0, 2)  # [length, batch_size, c]
        ...
        for l, (feat, pos_l) in enumerate(zip(features[-3:], pos[-3:])):
            ...
            if self.add_negative:
                neg_src_proj_l = self.fusion_module(tgt=src_proj_l,
                                                    memory=neg_text_word_features,
                                                    memory_key_padding_mask=neg_text_word_masks,
                                                    pos=neg_text_pos,
                                                    query_pos=None
                                                    )
                neg_src_proj_l = rearrange(neg_src_proj_l, '(t h w) b c -> (b t) c h w', t=t, h=h, w=w)
                neg_srcs.append(neg_src_proj_l)
        ...
        if self.num_feature_levels > (len(features) - 1):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                ...
                if self.add_negative:
                    neg_src = self.fusion_module(tgt=src,
                                                        memory=neg_text_word_features,
                                                        memory_key_padding_mask=neg_text_word_masks,
                                                        pos=neg_text_pos,
                                                        query_pos=None
                                                        )
                    neg_src = rearrange(neg_src, '(t h w) b c -> (b t) c h w', t=t, h=h, w=w)
                    neg_srcs.append(neg_src)
        ...
        segm = (self.fpn_type, self.use_cycle, self.pixel_decoder, features, text_features, text_sentence_features, pos, t, b, self.num_queries)
        neg = (neg_srcs, neg_text_features) if self.add_negative else None
        hs, memory, init_reference, inter_references, enc_outputs_class,\
        enc_outputs_coord_unact, inter_samples, mask_features, pseudo_sentence_feature, neg_pseudo_sentence_feature, VQ_loss = \
            self.transformer(srcs, text_embed, masks, poses, query_embeds, segm, neg)
        ...
        # # Segmentation
        # if self.fpn_type == 'dyn':
        #     mask_features = self.pixel_decoder(features, text_features, text_sentence_features, pos, memory,
        #                                        nf=t)  # [batch_size*time, c, out_h, out_w]
        # elif 'dual' in self.fpn_type:
        #     mask_features = self.pixel_decoder(features, text_features, text_sentence_features, pos, memory,
        #                                        nf=t)  # [batch_size*time, c, out_h, out_w]
        # else:
        ...
        out['sentence_feature'] = text_sentence_features
        if self.use_cycle:
            out['pseudo_sentence_feature'] = pseudo_sentence_feature
            if self.is_eval and self.neg_cls:
                pos_sentence_features = torch.cat(
                    (text_sentence_features, pseudo_sentence_feature.squeeze(0)), dim=-1)
                pair_sentence_features = torch.cat((pos_sentence_features,), dim=0)
                pair_logits = self.neg_classifier(pair_sentence_features)
                out['pair_logits'] = F.sigmoid(pair_logits)
                if F.sigmoid(pair_logits).item() < 0.5:
                    self.neg_count += 1
                    # print(F.sigmoid(pair_logits), self.neg_count)
            if self.add_negative:
                out['neg_sentence_feature'] = neg_text_sentence_features
                out['neg_pseudo_sentence_feature'] = neg_pseudo_sentence_feature
                if self.neg_cls:
                    pos_sentence_features = torch.cat(
                        (text_sentence_features, pseudo_sentence_feature.squeeze(0)), dim=-1)
                    neg_sentence_features = torch.cat(
                        (neg_text_sentence_features, neg_pseudo_sentence_feature.squeeze(0)), dim=-1)
                    pair_sentence_features = torch.cat((pos_sentence_features, neg_sentence_features), dim=0)
                    pair_logits = self.neg_classifier(pair_sentence_features)
                    pair_gt = torch.tensor([[0], [1]], dtype=torch.float32, device=pair_logits.device)
                    out['pair_logits'] = pair_logits
                    out['pair_gt'] = pair_gt
            # if self.add_negative or self.is_eval:
            #     out['negative_anchor'] = self.negative_anchor.weight
            if self.bert_cycle:
                out['sentence_feature'] = ori_text_sentence_features
                pseudo_sentence_feature = self.cycle_projector(pseudo_sentence_feature)
                out['pseudo_sentence_feature'] = pseudo_sentence_feature
                if self.add_negative:
                    out['neg_sentence_feature'] = ori_neg_text_sentence_features
                    neg_pseudo_sentence_feature = self.cycle_projector(neg_pseudo_sentence_feature)
                    out['neg_pseudo_sentence_feature'] = neg_pseudo_sentence_feature
        out['loss_VQ'] = VQ_loss
    ...
    def forward_text(self, captions, device):
        if isinstance(captions[0], str):
            ...
            ori_text_sentence_features = text_sentence_features
        ...
        return text_features, text_sentence_features, ori_text_sentence_features
...
class FeatureProjector(nn.Module):
    ...
    def __init__(self, input_feat_size, output_feat_size, do_ln=True):
        super().__init__()
        self.do_ln = do_ln
        # Object feature encoding
        self.fc1 = nn.Linear(input_feat_size, output_feat_size, bias=True)
        self.fc2 = nn.Linear(output_feat_size, output_feat_size, bias=True)
        self.layer_norm = nn.LayerNorm(output_feat_size, eps=1e-12)
        self.relu = nn.ReLU()

    def forward(self, encoder_features):
        x = self.fc1(encoder_features)
        if self.do_ln:
            x = self.layer_norm(x)
        x = self.relu(x)
        output = self.fc2(x)
...
def build(args):
    ...
    elif 'amm' in args.backbone:
        backbone = build_amm_backbone(args)
    else:
    ...
    model = ReferFormer(
        ...
        as_vos=args.as_vos,
        query_feat_dim=args.query_feat_dim,
        text_enc_type=args.text_enc_type,
        use_cycle=args.use_cycle,
        add_negative=args.add_negative,
        is_eval=args.is_eval,
        bert_cycle=args.bert_cycle,
        neg_cls=args.neg_cls,
    )
    ...
    if args.use_cycle:
        weight_dict['loss_cycle_dist'] = args.cycle_loss_dist_coef
        weight_dict['loss_cycle_angle'] = args.cycle_loss_angle_coef
        weight_dict['loss_cycle_mse'] = args.cycle_loss_mse_coef
    if args.add_negative:
        weight_dict['loss_cycle_contrastive'] = args.cycle_loss_contrastive_coef
    if args.neg_cls:
        weight_dict['loss_cycle_cls'] = args.cycle_loss_cls_coef
    if args.use_fg_contra:
        weight_dict['loss_fg_contra'] = args.fg_contra_loss_coef
    if args.quantitize_query:
        weight_dict['loss_VQ'] = args.VQ_loss_coef
    # TODO this is a hack
    if args.aux_loss:
        aux_weight_dict = {}
        for i in range(args.dec_layers - 1):
            aux_weight_dict.update({k + f'_{i}': v for k, v in weight_dict.items() if 'cycle' not in k})
    ...
    criterion = SetCriterion(
            ...
            margin=args.contra_margin,
            use_cycle=args.use_cycle,
            add_negative=args.add_negative,
            neg_cls=args.neg_cls,
            quantitize_query=args.quantitize_query,
            )
    criterion.to(device)
