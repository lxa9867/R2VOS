...
# from models.referformer import FeatureResizer
from models.vector_quantitizer import VectorQuantizerEMA, VectorQuantizer
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 two_stage=False, two_stage_num_proposals=300, mix_query=False, quantitize_query=False,
                 freeze_quantitizer=False):
        super().__init__()
        ...
        if self.mix_query:
            self.query_mixer = FeatureResizer(
                input_feat_size=d_model * 2,
                output_feat_size=d_model,
                dropout=0.0,
                )
        if self.quantitize_query:
            self.query_quantitizer = VectorQuantizer(num_embeddings=512, embedding_dim=d_model)
            if freeze_quantitizer:
                for p in self.query_quantitizer.parameters():
    ...
    def forward(self, srcs, tgt, masks, pos_embeds, query_embed=None, segm=None, neg=None):
        ...
        if neg:
            neg_srcs, neg_text_features = neg
            neg_src_flatten = []
            for neg_src in neg_srcs:
                neg_src = neg_src.flatten(2).transpose(1, 2)
                neg_src_flatten.append(neg_src)
            neg_src_flatten = torch.cat(neg_src_flatten, 1)
        ...
        memory = self.encoder(src_flatten, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten,
                              mask_flatten)
        if neg:
            neg_memory = self.encoder(neg_src_flatten, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten, mask_flatten)
        ...
        if neg:
            ...
            for lvl in range(self.num_feature_level - 1):
                h, w = spatial_shapes[lvl]
                # [bs*t, c, h, w]
                neg_memory_lvl = neg_memory[:, neg_spatial_index: neg_spatial_index + h * w, :].reshape(bs, h, w, c).permute(0, 3, 1,
                                                                                                             2).contiguous()
                neg_spatial_index += h * w
        # Segmentation
        # prepare segm
        fpn_type, use_cycle, pixel_decoder, features, text_features, text_sentence_features, pos, t, b, num_queries = segm

        if fpn_type == 'dyn':
            mask_features = pixel_decoder(features, text_features, text_sentence_features, pos, memory_features,
                                               nf=t)  # [batch_size*time, c, out_h, out_w]
        elif 'dual' in fpn_type:
            if use_cycle:
                if neg:
                    mask_features, out_feat, pseudo_sentence_feature, neg_pseudo_sentence_feature = pixel_decoder(
                        features, text_features,
                        text_sentence_features, pos,
                        memory_features,
                        nf=t, neg_memory=neg_memory_lvl, neg_text_features=neg_text_features)
                else:
                    # [batch_size*time, c, out_h, out_w]
                    mask_features, out_feat, pseudo_sentence_feature = pixel_decoder(features, text_features,
                                                                                     text_sentence_features, pos,
                                                                                     memory_features,
                                                                                     nf=t)
            else:
                mask_features, out_feat = pixel_decoder(features, text_features, text_sentence_features, pos, memory_features,
                                               nf=t)  # [batch_size*time, c, out_h, out_w]
        else:
            mask_features = pixel_decoder(features, text_features, pos, memory_features,
                                               nf=t)  # [batch_size*time, c, out_h, out_w]
        mask_features = rearrange(mask_features, '(b t) c h w -> b t c h w', b=b, t=t)

        if 'memory_fpn' in fpn_type:
            memory_fpn = []
            # out_feat: [32x, 16x, 8x], bchw
            for feat in out_feat[::-1]:
                feat = feat.permute(0, 2, 3, 1).flatten(1, 2)
                memory_fpn.append(feat)
            memory_fpn = [memory[:, spatial_index:, :]] + memory_fpn
            memory = torch.cat(memory_fpn, dim=1)

        # prepare input for decoder
        bs, _, c = memory.shape
        b, t, q, c = tgt.shape
        tgt = rearrange(tgt, 'b t q c -> (b t) q c')
        if use_cycle and self.mix_query:
            pseudo_tgt = repeat(pseudo_sentence_feature.squeeze(0), 'b c -> b t q c', t=t, q=q)
            pseudo_tgt = rearrange(pseudo_tgt, 'b t q c -> (b t) q c')
            tgt_ = torch.cat((tgt, pseudo_tgt), dim=-1)
            tgt = tgt + self.query_mixer(tgt_)
        if self.quantitize_query:
            VQ_loss, tgt, _, _ = self.query_quantitizer(tgt)
        ...
        return hs, memory_features, init_reference_out, inter_references_out, None, None, inter_samples, mask_features, \
               pseudo_sentence_feature if use_cycle else None, neg_pseudo_sentence_feature if use_cycle and neg else None,\
               VQ_loss if self.quantitize_query else None
...
def build_deforamble_transformer(args):
    return DeformableTransformer(
        ...
        mix_query=args.mix_query,
        quantitize_query=args.quantitize_query,
        freeze_quantitizer=args.freeze_quantitizer,
    )
